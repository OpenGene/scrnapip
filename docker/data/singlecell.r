...
if (isstep(3)){
  ...
  if(csc$step3$recluster & length(table(pbmcall$orig.ident))> 1 ){
    library(harmony)
    library(scran)
    library(dplyr)
    library(batchelor)
  ...
  if(csc$step3$clustercell){
    ...
    if(csc$step3$doubletfinder){
      ...
      loginfo("DoubletFinder start")
      if(csc$step3$doublesct){
	  
      sweep.res.list_kidney <- paramSweep_v3(pbmcall, PCs = 1:10, sct = csc$step3$doublesct)
	  }else{
	    pbmcall@active.assay="RNA"
	  pbmcall =pbmcall %>% NormalizeData() %>% FindVariableFeatures() %>% ScaleData() %>% RunPCA()
	  sweep.res.list_kidney <- paramSweep_v3(pbmcall, PCs = 1:10, sct = csc$step3$doublesct)
  
	  }
      #gt.calls <- pbmcall@meta.data[rownames(sweep.res.list_kidney[[1]]), "GT"]. 
      
      sweep.stats_kidney <- summarizeSweep(sweep.res.list_kidney, GT = F)
      bcmvn_kidney <- find.pK(sweep.stats_kidney)
      homotypic.prop <- modelHomotypic(pbmcall$seurat_clusters)   ## ex: annotations <- seu_kidney@meta.data$ClusteringResults
      pK_bcmvn <- bcmvn_kidney$pK[which.max(bcmvn_kidney$BCmetric)] %>% as.character() %>% as.numeric()
      
      
      DoubletRate = csc$step3$doublerate %>% as.numeric()
      
      nExp_poi <- round(DoubletRate*nrow(pbmcall@meta.data))  ## Assuming 7.5% doublet formation rate - tailor for your dataset
      nExp_poi.adj <- round(nExp_poi*(1-homotypic.prop))
      
      ## Run DoubletFinder with varying classification stringencies ----------------------------------------------------------------
      pbmcall <- doubletFinder_v3(pbmcall, PCs = 1:10, pN = 0.25, pK = pK_bcmvn, nExp = nExp_poi.adj, reuse.pANN = FALSE, sct = csc$step3$doublesct)
      #pbmcall <- doubletFinder_v3(pbmcall, PCs = 1:10, pN = 0.25, pK = pK_bcmvn, nExp = nExp_poi.adj, reuse.pANN = "pANN_0.25_0.09_913", sct = F)
      
      doubleplot=DimPlot(pbmcall,group.by = colnames(pbmcall@meta.data) %>% tail(1))
      savepdf(paste(outdir,"step3",cscumap,"plotby_DoubletFinder",sep="/"),doubleplot,widthle = 10,heightle = 8)
      write.table(pbmcall@meta.data,paste(outdir,"step3",cscumap,"DoubletFinder.txt",sep="/"),col.names = T,row.names = T,sep = "\t",quote = F)      #tail(colnames(pbmcall@meta.data),2)[1]
      #FeaturePlot(pbmcall,features = tail(colnames(pbmcall@meta.data),2)[1])
